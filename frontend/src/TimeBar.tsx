 ...
 * 30 días. `fecha` null = en vivo; "YYYY-MM-DD" = modo archivo. */
  ...
  return (
    <div className="timebar" role="group" aria-label="Viaje en el tiempo">
      ...
      <div className="tb-estado">
        {fecha ? (
          <>
            <span className="tb-modo archivo">⏪ ARCHIVO</span>
      ...
      {fecha && (
        <div className="tb-nota">
          satélites: día seleccionado · AIS: {HAY_BACKEND ? "estado de ese día (retención 14 d)" : "requiere backend"} ·
          aeronaves: solo en vivo
        </div>
      )}
    </div>
  );
